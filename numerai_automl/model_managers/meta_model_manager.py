...
class MetaModelManager:
    ...
    def create_neutralized_predictions(self, X: pd.DataFrame):
        X = X[self.features]
        neutralized_predictions = self.base_model_manager.create_neutralized_predictions_from_by_models_predictions()
    ...
    def create_weighted_meta_model(self):
        ...
        def predict(self, X: pd.DataFrame) -> pd.Series:
            ...
            for target_name in self.targets_names_for_base_models:
                ...
                vanila_predictions = base_model.predict(X)
                neutralized_predictions = self.feature_neutralizer.neutralize_predictions(vanila_predictions, target_name)
