...
class DataLoader:
    ...
    def load_train_data(self, feature_set: str = "medium", target_set: List[str] = ["target"], downsample_step: int = 4, start_index: int = 0) -> pd.DataFrame:
        ...
        return self._downsample_data(train, downsample_step, start_index)

    def load_validation_data(self, feature_set: str = "medium", target_set: List[str] = ["target"], downsample_step: int = 4, start_index: int = 0) -> pd.DataFrame:
        features = self.feature_sets[feature_set]
        ...
        return self._downsample_data(validation, downsample_step, start_index)
    ...
    def _downsample_data(self, df: pd.DataFrame, downsample_step: int, start_index: int = 0) -> pd.DataFrame:
        """Helper method to downsample data by era"""
        if downsample_step > 1:
            unique_eras = df["era"].unique()[start_index::downsample_step]
